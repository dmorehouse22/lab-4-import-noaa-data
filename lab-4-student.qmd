---
title: "Lab 4: Importing NOAA Data"
...
Throughout Monterey Bay, there are various NOAA stations that capture data on wind speed, wave height, air temperature and many other data points. Data such as this can be used to predict weather or tidal event or used in conjunction with other data, such as population estimates of sardines or whale migration, to understand the ocean ecosystem.

::::: {layout="[ 70, 30 ]"}
::: {#first-column}
For this lab, we will focus on [Station 46092](https://www.ndbc.noaa.gov/station_history.php?station=46092), which is maintained by MBARI (Monterey Bay Aquarium Research Institute) on the outer edge of the Monterey Bay Canyon. Data is collected and stored hourly, but sometimes there are issues with censors not working or other events that lead to missing data.
:::
...
buoy_2024 <- read_table("data_raw/buoy_2024.txt", comment = "#")
```
...
buoy_2024 <- janitor::clean_names(buoy_2024)
```
...
write_csv(buoy_2024, file = "data_raw/buoy_update.csv")
```
...
buoy_update <- read_csv("data_raw/buoy_update.csv", 
...
write_csv(buoy_final, file = "data_clean/buoy_final.csv")
```
...
buoy_final <- read_csv("data_clean/buoy_final.csv")

ggplot(data = buoy_final) + 
  geom_jitter(mapping = aes(x = factor(mm), y = atmp, color = factor(mm)), width = 0.05, alpha = 0.1) +
  geom_smooth(mapping = aes(x = mm, y = atmp), color = "black") + 
  theme_light() + 
...
The data set I selected to clean and analyze is from my independent research project this past fall on thermal stress response in marine sponges. I observed the difference in thermal stress response between a species of sponge with an active chemical defense and one without. In this particular file, I observed the respiration rates of sponge fragments through incubation. 

Some questions this data could answer may be...

1. How does the oxygen per hour rates differ between the 2 sponge species in the ambient condition?

2. How does the oxygen per hour rates differ between the 2 sponge species in the heated condition?
